...
public class ExactProcessor implements Processor
{
    private TransactionStore transactionStore;
    private ExpenseStore expenseStore;

    public ExactProcessor(TransactionStore transactionStore, ExpenseStore expenseStore)
    {
    ...
    public Result process(Command command)
    {
        ...
        for(int count = 0; count < listOfUsers.size(); count++)
        {
            ...
            {
                Transaction transaction = new Transaction(count, sender, user, expense);
